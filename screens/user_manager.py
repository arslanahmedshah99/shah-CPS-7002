...
def users_tab_layout():
    ...
    users = read_users()
    return html.Div([
        html.H3("Users Management", className="mb-4 text-primary fw-bold"),

        html.Div(id="users-table-container"),

        dbc.Button("Add User", id="add-user-btn", color="primary", className="me-2 mb-3 px-4 py-2 fw-semibold"),
    ...
    def render_table(users):
        ...
        for user in users:
            rows.append(html.Tr([
                ...
                html.Td([
                    dbc.Button("Edit", id={"type": "edit-user", "id": user["id"]}, size="sm", className="btn-edit me-1"),
                    dbc.Button("Delete", id={"type": "delete-user", "id": user["id"]}, size="sm", className="btn-delete"),
                ])
            ]))
